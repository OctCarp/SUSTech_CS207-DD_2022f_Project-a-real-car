...
预留有启动控制模块，分别控制小车的电源开、关与模式选择。有手动模式有油门、挂档、换挡、刹车与转向功能及控制，半自动模式有前进与转向的控制，及胡同自动转向和路口等待指令的功能，自动模式有自动寻至出口的功能。
...
- **半自动模式：**
  ...
  - **行进逻辑：**离开起点且未遇到岔路口时，小车将自动寻路，在检测到死胡同后调转 180 度后前进。小车遇到只能左转或者只能右转的情况也会自动左转并前进或右转并前进。
  - **前进指令：**遇到岔路口时，推动油门开关一次，小车将前进。
  - **倒转指令：**遇到岔路口时，推动倒挡开关一次，小车将调转 180 度后前进。
  - **左转指令：**遇到岔路口时，按下左转按钮，小车将左转。
  - **右转指令：**遇到岔路口时，按下左转按钮，小车将右转。
- **自动模式：**
...
- 郭：
  - 全自动驾驶时如果遇到右转后进入一个要不断右转的回路，则小车会被永远困在回路中。
